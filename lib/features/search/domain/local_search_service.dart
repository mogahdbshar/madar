...
class LocalSearchResult {
  const LocalSearchResult({required this.type, required this.id, required this.title, required this.snippet});
  final String type, id, title, snippet;
}
class LocalSearchService {
  LocalSearchService(this.db);
  ...
  Future<List<LocalSearchResult>> search(String rawQuery) async {
    ...
    final hadith = await (db.select(db.hadithEntries)..where((t) => t.originalText.like('%$query%'))..limit(30)).get();
    ...
    final names = await (db.select(db.namesOfAllah)..where((t) => t.arabicName.like('%$query%'))..limit(20)).get();
