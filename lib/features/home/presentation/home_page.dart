 ...
 @override Widget build(BuildContext context)=>Scaffold(body:SafeArea(child:CustomScrollView(slivers:[
  ...
  SliverPadding(padding:const EdgeInsets.fromLTRB(18,12,18,28),sliver:SliverList(delegate:SliverChildListDelegate([
   MadarGlassCard(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    Text('اليوم',style:Theme.of(context).textTheme.labelLarge),const SizedBox(height:6),Text('الصلاة القادمة',style:Theme.of(context).textTheme.titleLarge),
    const SizedBox(height:4),const Text('اختر موقعك أو مدينة يدوية لعرض المواقيت'),const SizedBox(height:14),
   ...
   MadarSection(title:'العبادة',child:Column(children:[
    ...
    const SizedBox(height:10),MadarFeatureTile(icon:Icons.auto_awesome_rounded,title:'التسبيح',subtitle:'جلسة عداد تحفظ تقدمك محلياً',onTap:()=>context.push('/tasbeeh')),
   ])),
  ]))),]));
}
